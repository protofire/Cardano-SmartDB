...
2.1. Entity Retrieval:
   - The system retrieves the current state of the entity from the database.
   - It checks if the entity exists and if the user has the right to update it.

2.2. UTXO Selection:
   - The system queries available UTXOs for the transaction.
...
6.2. Database Synchronization:
   - The system initiates a sync process for the affected entity.

6.3. SmartDB Entity Update:
   - The corresponding SmartDB entity in the database is updated to reflect the new state from the blockchain.
...
7.2. Cleanup:
   - Any temporary locks or reservations on UTXOs are cleared.
...
- UTXO Locking: The system uses timestamps (isPreparingForReading, isReading, isPreparingForConsuming, isConsuming) to manage UTXO access, preventing double-spending and read-after-consume scenarios.
...
- Transaction Failure: If a transaction fails at any stage, the system updates its status accordingly and releases any reserved UTXOs.
...
- User Feedback: The frontend is updated with transaction status, providing real-time feedback to users.
...
1. CREATED: Initial state when the transaction entity is first created in the database, used to reserve UTXOs.
...
- **Automatic Status Updater**: 
    - The status updater job is automatically initiated whenever a transaction is marked as `SUBMITTED`. 
...
- **Manual Job Initiation**:
    ...
    - Similarly, the synchronization (`sync`) process can be triggered manually if required, to force an immediate update for an entity or transaction.
